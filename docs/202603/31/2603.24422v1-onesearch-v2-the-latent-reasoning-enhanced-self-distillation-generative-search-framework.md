...
tags: ["query:cir-ml"]
score: 7.0
evidence: 现代搜索系统的生成式检索框架
tldr: OneSearch-V2是一个增强潜在推理和自蒸馏的生成式搜索框架，旨在解决工业级生成式检索中复杂查询理解不足、用户意图挖掘不深及偏好过拟合等问题。通过引入思维增强模块、推理内化自蒸馏流水线及行为偏好对齐优化，该框架在不增加推理延迟的前提下，显著提升了电商搜索的点击率、转化率和相关性，并有效缓解了信息茧房和长尾稀疏问题。
selection_source: fresh_fetch
motivation: 现有的生成式搜索框架在处理复杂查询、挖掘潜在意图以及克服历史偏好过拟合方面存在局限，限制了其在工业场景下的进一步性能提升。
method: 提出了包含思维增强查询理解、推理内化自蒸馏训练以及行为偏好对齐优化三大核心模块的OneSearch-V2框架。
result: "在线A/B测试显示点击率提升3.98%，转化率提升3.05%，同时在不增加推理成本的情况下显著改善了搜索体验和相关性。"
conclusion: OneSearch-V2通过将推理能力内化到生成模型中，成功实现了高效且精准的工业级生成式检索，为解决搜索系统中的长尾和茧房问题提供了新思路。
---

## 摘要
生成式检索（GR）已成为现代搜索系统的一种极具前景的范式。与多阶段级联架构相比，它具有端到端联合优化和高计算效率等优势。OneSearch 作为一种代表性的工业级部署生成式搜索框架，带来了显著的商业和运营效益。然而，它对复杂查询的理解不足、对潜在用户意图的挖掘效率低下，以及对狭隘历史偏好的过拟合，限制了其性能的进一步提升。为了应对这些挑战，我们提出了 OneSearch-V2，这是一种潜性推理增强的自蒸馏生成式搜索框架。它包含三项关键创新：（1）思维增强的复杂查询理解模块，能够实现深层查询理解，并克服直接推理的浅层语义匹配局限性；（2）推理内化的自蒸馏训练流水线，通过隐式上下文学习，在日志拟合之外揭示用户潜在且精准的电商意图；（3）行为偏好对齐优化系统，缓解了单一转化指标带来的奖励破解（reward hacking）问题，并通过直接用户反馈解决个人偏好。广泛的线下评估证明了 OneSearch-V2 强大的查询识别和用户画像能力。线上 A/B 测试进一步验证了其业务有效性，带来了 +3.98% 的商品点击率（CTR）、+3.05% 的买家转化率和 +2.11% 的订单量。人工评估进一步确认了搜索体验质量的提升，页面优良率提升了 +1.65%，查询-商品相关性提升了 +1.37%。更重要的是，OneSearch-V2 有效缓解了信息茧房和长尾稀疏等常见搜索系统问题，且未增加额外的推理成本或服务延迟。
