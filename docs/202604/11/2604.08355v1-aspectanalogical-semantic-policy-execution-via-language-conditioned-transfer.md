...
tldr: 针对强化学习智能体在结构相似任务中泛化能力差的问题，本文提出ASPECT框架。该方法利用文本条件的变分自编码器（VAE）和大型语言模型（LLM）作为动态语义算子。在测试时，LLM将新任务的观察描述重映射为源任务描述，通过VAE生成与原始策略兼容的想象状态，从而在无需重新训练的情况下实现跨复杂类比任务的零样本迁移。
selection_source: fresh_fetch
motivation: 现有的零样本迁移方法通常受限于预定义的离散类别系统，难以应对新颖或组合式的任务变体。
method: 利用LLM将当前观察的语义描述重映射至源任务空间，并结合文本条件VAE生成原始策略可执行的想象状态。
result: 该方法在多种复杂且新颖的类比任务中成功实现了零样本迁移，超越了传统固定类别映射的局限性。
conclusion: 通过引入LLM的灵活推理能力进行语义对齐，ASPECT显著增强了强化学习策略在语义相似任务间的泛化与复用能力。
---
