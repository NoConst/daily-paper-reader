...
tldr: 针对DeepSeek等模型中细粒度稀疏注意力（DSA）索引器在长文本下存在的O(L^2)计算瓶颈，本文提出HISA。HISA采用分层索引策略，通过块级粗过滤和令牌级精细化两阶段过程，在无需重新训练的情况下显著提升了检索效率。实验表明，HISA在保持极高选择保真度的同时，大幅降低了长序列推理延迟，为长文本高效推理提供了新方案。
selection_source: fresh_fetch
motivation: 现有的细粒度稀疏注意力索引器需要扫描全部历史令牌，导致长文本场景下计算开销呈平方级增长，成为系统性能瓶颈。
method: 提出一种两阶段分层索引机制，先通过块级池化表示进行粗筛选以剔除无关区域，再在候选块内进行精确的令牌级评分。
result: "在128K上下文长度下实现了4倍的内核级加速，且与原始DSA的令牌选择交并比超过99%，在长文本任务中保持了极高的精度。"
conclusion: HISA作为一种无需微调的即插即用方案，在几乎不损失模型性能的前提下，有效解决了细粒度稀疏注意力在大规模上下文推理中的扩展性难题。
---
