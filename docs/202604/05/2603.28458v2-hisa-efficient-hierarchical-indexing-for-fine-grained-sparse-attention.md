...
tldr: 针对细粒度稀疏注意力（如DeepSeek DSA）中索引器需扫描全量历史Token导致的计算瓶颈，本文提出HISA。HISA通过两阶段层次化索引（块级粗过滤和Token级精细筛选）重写搜索路径，在无需重新训练的情况下，显著提升了长文本场景下的推理效率，同时保持了与原始模型相当的性能。
selection_source: fresh_fetch
motivation: 现有的细粒度稀疏注意力索引器在长文本下需扫描所有历史Token，成为制约推理速度的性能瓶颈。
method: 提出一种分层索引机制，先通过池化后的块表示进行粗筛选，再在保留的候选块内进行细粒度的Token级索引。
result: 在64K上下文下实现了显著的内核级加速，且在DeepSeek-V3.2和GLM-5上的测试显示其性能接近原始DSA并优于块稀疏基准。
conclusion: HISA是一种高效、即插即用的稀疏注意力优化方案，能在不损失精度且无需微调的前提下解决长文本索引瓶颈。
---

## 摘要
以 DeepSeek 稀疏注意力 (DSA) 为代表的 Token 级稀疏注意力机制，通过轻量级索引器对每个查询 (query) 的所有历史键 (key) 进行评分来实现细粒度的键选择，并仅在选定子集上计算注意力。虽然下游稀疏注意力本身具有良好的扩展性，但索引器仍需为每个查询扫描整个前缀，这引入了随上下文长度增长而变得难以承受的逐层瓶颈。我们提出了 HISA（分层索引稀疏注意力），这是一种索引器的即插即用替代方案，它将搜索路径从扁平的 Token 扫描重写为两阶段的分层过程：(1) 块级粗筛选阶段，通过对池化后的块表示进行评分来剔除无关区域；(2) Token 级精炼阶段，仅在保留的候选块内应用原始索引器。HISA 保留了下游 Sparse MLA 算子所使用的完全相同的 Token 级 Top-sparse 模式，且无需额外训练。在算子级基准测试中，HISA 在 64K 上下文下实现了显著加速。在 Needle-in-a-Haystack 和 LongBench 评测中，我们在无需任何微调的情况下，直接将 DeepSeek-V3.2 和 GLM-5 中的索引器替换为 HISA 索引器。实验表明，HISA 在质量上与原始 DSA 紧密匹配，同时性能大幅超越了块稀疏 (block-sparse) 基准方案。
