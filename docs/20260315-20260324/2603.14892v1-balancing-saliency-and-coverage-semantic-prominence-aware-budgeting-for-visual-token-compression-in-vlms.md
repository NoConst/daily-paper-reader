...
## 速览
**TLDR**：针对大视觉语言模型（VLM）中高分辨率输入导致的计算瓶颈，本文提出 PromPrune 框架。该方法观察到不同样本的语义显著性分布存在差异，因此摒弃了静态压缩策略，转而采用样本自适应的视觉标记选择。通过两阶段流水线动态分配局部显著区域与全局多样性区域的标记预算，PromPrune 在大幅降低计算开销的同时，有效保持了模型的理解能力。 \
**Motivation**：现有的视觉标记压缩方法多采用静态策略，忽略了不同样本间语义显著性分布的差异，导致在局部显著性与全局覆盖度之间的权衡并非最优。 \
**Method**：提出 PromPrune 框架，通过语义显著性感知预算分配和两阶段选择流水线，根据样本特征自适应地平衡局部显著性保留与全局覆盖。 \
**Result**：在 LLaVA-NeXT-7B 模型上，该方法在减少 88% 的 FLOPs 和 22% 的预填充延迟的同时，保留了 97.5% 的原始准确率。 \
**Conclusion**：样本自适应的标记预算分配是实现高效且高性能视觉标记压缩的关键，能有效缓解 VLM 的计算瓶颈。
