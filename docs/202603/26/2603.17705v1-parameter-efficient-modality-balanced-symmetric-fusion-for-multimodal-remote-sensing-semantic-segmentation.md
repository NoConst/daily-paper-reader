...
title_zh: 面向多模态遥感语义分割的参数高效且模态平衡的对称融合框架
...
## 摘要
多模态遥感语义分割通过利用来自异构数据的互补物理线索来增强场景解释能力。尽管预训练视觉基础模型（VFMs）提供了强大的通用表示，但将其适配到多模态任务通常会带来巨大的计算开销，且容易出现模态不平衡问题，即辅助模态的贡献在优化过程中被抑制。为了应对这些挑战，我们提出了 MoBaNet，一种参数高效且模态平衡的对称融合框架。MoBaNet 构建在大部分冻结的 VFM 骨干网络之上，采用对称双流架构，在最小化可训练参数数量的同时保留了可泛化的表示。具体而言，我们设计了一个跨模态提示注入适配器（CPIA），通过生成共享提示并将其注入冻结骨干网络下的瓶颈适配器中，实现深层语义交互。为了获得用于解码的紧凑且具有判别性的多模态表示，我们进一步引入了差异引导门控融合模块（DGFM），该模块通过显式利用跨模态差异来引导特征选择，从而自适应地融合配对的阶段特征。此外，我们提出了一种模态条件随机掩码（MCRM）策略，通过仅在训练期间对单一模态进行掩码，并在模态特定分支上施加硬像素辅助监督，来缓解模态不平衡。在 ISPRS Vaihingen 和 Potsdam 基准数据集上的广泛实验表明，MoBaNet 以远少于全量微调的可训练参数实现了最先进的性能，验证了其在鲁棒且平衡的多模态融合方面的有效性。本工作的源代码可在 https://github.com/sauryeo/MoBaNet 获取。
