...
tldr: 针对多模态推荐中原始视觉特征缺乏高层语义的问题，本文提出VLM4Rec框架。该方法不再依赖复杂的特征融合，而是利用大视觉语言模型（LVLM）将商品图像转化为自然语言描述，实现语义对齐。通过将这些描述编码为稠密向量并进行语义匹配，VLM4Rec在多个数据集上显著提升了推荐性能，证明了高质量语义表示在推荐系统中的关键作用。
selection_source: fresh_fetch
motivation: 传统的视觉特征主要捕捉外观相似性，难以反映用户决策背后如风格、材质和使用场景等高层语义因素。
method: 利用LVLM将图像转化为文本描述，并将其编码为稠密向量，通过简单的基于画像的语义匹配机制实现离线与在线解耦的推荐。
result: 实验结果显示，VLM4Rec在多个多模态推荐基准测试中均优于基于原始视觉特征和复杂融合机制的现有方法。
conclusion: 研究表明，在多模态推荐中，通过语义对齐提升表示质量比增加融合算法的复杂度更为重要且有效。
---
