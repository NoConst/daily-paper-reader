...
tags: ["query:cir-ml"]
score: 6.0
evidence: 多模态大语言模型中的内存使用
tldr: 针对多模态大模型在处理高分辨率图像或长视频时KV缓存导致的显存瓶颈，本文提出一种顺序输入压缩机制。该方法利用视觉Token的结构冗余性，在预填充阶段即执行结构感知的KV缓存压缩，从而在维持生成性能的同时显著降低峰值显存占用，提升了多模态推理的效率与实用性。
source: arxiv
...
motivation: 现有压缩方法多在输入处理完成后执行，导致预填充阶段峰值显存过高，限制了模型处理大规模视觉数据的能力。
method: 提出一种在预填充过程中实施的顺序输入压缩机制，通过结构感知的KV缓存压缩来强制执行固定的显存预算。
result: 实验证明该方法大幅降低了推理时的峰值显存需求，且对模型生成性能的影响微乎其微。
conclusion: 在预填充阶段进行实时显存管理是实现高效、低资源消耗多模态大模型推理的关键路径。
---
