...
tldr: "本研究对比了基于嵌入和生成式模型在地球科学技术文档分类中的表现。通过多学科基准测试，评估了模型准确性、稳定性和成本。结果显示，结合思维链（CoT）的生成式视觉语言模型（如Qwen2.5-VL）在零样本准确率上显著优于多模态嵌入模型（82% vs 63%）。此外，研究探讨了监督微调对提升性能的作用及其对数据不平衡的敏感性。"
selection_source: fresh_fetch
motivation: 旨在评估并对比嵌入式与生成式大模型在处理复杂多学科技术文档分类任务时的效能、稳定性和成本权衡。
method: 利用地球科学基准数据集，对比分析了Qwen2.5-VL等生成式模型与QQMM等嵌入式模型在零样本及微调场景下的分类表现。
result: "带有思维链（CoT）的生成式模型以82%的零样本准确率显著优于嵌入式模型的63%，证明了其在复杂文档理解上的优势。"
conclusion: 生成式模型在文档分类中展现出更强的零样本能力，但在进行监督微调时需特别注意训练数据分布不均对模型性能的影响。
---
