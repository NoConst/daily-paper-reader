...
tldr: 针对边缘AI设备在非平稳数据流中进行持续学习时面临的计算和内存瓶颈，本文提出了ImageHD。这是一种基于超维计算（HDC）的FPGA加速器，通过量化CNN提取特征并利用HDC进行快速非迭代更新。ImageHD引入了硬件感知的聚类管理策略，并在AMD Zynq FPGA上实现了流式数据流架构，显著提升了处理速度和能效，为实时边缘持续学习提供了高效方案。
source: arxiv
...
motivation: 现有的持续学习方法依赖高开销的反向传播或复杂的内存管理，难以在资源受限的边缘设备上实现高效部署。
method: 提出一种基于FPGA的加速器架构，结合量化CNN前端、硬件感知的有界聚类管理策略以及基于字打包二进制超向量的并行位运算。
result: 在CORe50数据集上，ImageHD相比优化后的CPU和GPU基准，分别实现了高达40.4倍和4.84倍的加速，以及383倍和105.1倍的能效提升。
conclusion: 该研究证明了利用超维计算在资源受限的硬件上实现实时、低功耗边缘持续学习的可行性与优越性。
---

## 摘要
设备端持续学习（CL）对于在非平稳数据流上运行的边缘人工智能系统至关重要，但大多数现有方法依赖于反向传播或重样本分类器，导致了巨大的计算、内存和延迟开销。超维计算（HDC）通过快速、非迭代的在线更新提供了一种轻量级的替代方案。结合紧凑型卷积神经网络（CNN）特征提取器，HDC 能够利用强大的视觉表示实现高效的设备端自适应。然而，先前的基于 HDC 的持续学习系统通常依赖于多级内存层级和复杂的聚类管理，限制了其在资源受限硬件上的部署能力。我们提出了 ImageHD，这是一款基于 HDC 的视觉数据设备端持续学习 FPGA 加速器。ImageHD 针对严格延迟和片上内存限制下的流式持续学习，避免了昂贵的迭代优化。在算法层面，我们引入了一种硬件感知的持续学习方法，通过统一的样本内存和硬件高效的聚类合并策略来限制类别样本，同时结合量化 CNN 前端以在不牺牲准确性的情况下减少部署开销。在系统层面，ImageHD 在 AMD Zynq ZCU104 FPGA 上实现为流式数据流架构，集成了 HDC 编码、相似度搜索和有界聚类管理，利用字打包二进制超向量在紧凑的片上资源预算内进行大规模并行位运算。在 CORe50 数据集上，ImageHD 相比优化的 CPU（GPU）基准测试分别实现了高达 40.4 倍（4.84 倍）的加速和 383 倍（105.1 倍）的能效提升，证明了基于 HDC 的持续学习在实时边缘人工智能中的实用性。
