...
tldr: 针对多视图学习中维度严重失衡的问题（如视频与生理信号），本文提出AdaMuS框架。该框架通过特定视图编码器将不同维度特征映射至统一空间，并设计无参数剪枝方法以减少低维视图映射时的过拟合。同时，引入稀疏融合范式抑制冗余并对齐视图，结合自监督学习增强泛化能力。实验证明该方法在分类和语义分割任务上均表现优异。
selection_source: fresh_fetch
motivation: 传统多视图学习方法在处理维度极度失衡的数据时，容易偏向高维视图并导致低维视图表示出现严重冗余。
method: 提出一种自适应多视图稀疏学习框架，结合无参数剪枝编码器、稀疏融合范式以及基于相似性图的自监督学习。
result: 在合成数据集和七个真实世界基准测试中，AdaMuS在分类和语义分割任务上均取得了领先的性能。
conclusion: AdaMuS有效地解决了多视图学习中的维度失衡挑战，通过稀疏化策略提升了模型的对齐效果和泛化性能。
---

## 摘要
多视图学习的主要目标是融合多种特征以全面地描述数据。大多数先前的研究隐含地假设不同视图具有相似的维度。然而，在实践中，不同视图之间往往存在严重的维度差异，从而导致不平衡的多视图学习问题。例如，在情感识别任务中，视频帧的维度通常达到 $10^6$，而生理信号仅包含 $10^1$ 个维度。针对这一问题，现有方法通常面临两个主要挑战：（1）它们往往偏向高维数据，而忽略了低维视图；（2）在极端维度不平衡的情况下，它们难以有效地对齐表示，这会给低维表示引入严重的冗余。为了解决这些问题，我们提出了自适应多视图稀疏学习（AdaMuS）框架。首先，为了防止忽略低维视图的信息，我们构建了特定于视图的编码器，将其映射到统一的维度空间。鉴于将低维数据映射到高维空间通常会导致严重的过拟合，我们设计了一种无参数剪枝方法，以自适应地移除编码器中的冗余参数。此外，我们提出了一种稀疏融合范式，能够灵活地抑制冗余维度并有效地对齐各个视图。此外，为了学习具有更强泛化能力的表示，我们提出了一种自监督学习范式，通过构建相似度图来获取监督信息。在合成玩具数据集和七个真实世界基准测试上的广泛评估表明，AdaMuS 在分类和语义分割任务中均能持续取得优异的性能，并表现出强大的泛化能力。
