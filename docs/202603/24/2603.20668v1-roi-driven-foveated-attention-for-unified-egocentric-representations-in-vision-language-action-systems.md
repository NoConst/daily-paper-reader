...
tldr: 针对具身智能中视觉-语言-动作（VLA）模型面临的数据采集成本高、跨机器人对齐难等挑战，本文提出了一种基于感兴趣区域（ROI）驱动的工程工作流。该方法通过正向运动学将末端执行器位姿投影至外部相机，生成以手部为中心的区域，在保留全局上下文的同时维持了关键接触区域的高信息密度。该工作流实现了跨异构机器人的数据归一化与复用，为大规模感知与特定控制之间搭建了桥梁。
selection_source: fresh_fetch
motivation: 现有的VLA模型受限于高昂的数据采集成本、有限的跨机器人对齐能力以及互联网视觉数据向机器人控制迁移的困难。
method: 通过正向运动学将机器人末端位姿投影到外部相机视图中，提取以手部为中心的ROI，并结合全局上下文构建统一的自我中心表征。
result: 建立了一套包含标定、同步和ROI生成的完整流水线，实现了跨异构机器人的具身对齐和视点归一化数据表征。
conclusion: ROI驱动的自我中心表征是实现大规模数据采集和跨机器人学习的实用抽象，有效促进了感知与控制的融合。
---
