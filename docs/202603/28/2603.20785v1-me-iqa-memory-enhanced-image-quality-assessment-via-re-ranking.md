...
tldr: 针对推理型视觉语言模型（VLM）在图像质量评估（IQA）中存在的得分不敏感及“离散坍缩”问题，本文提出ME-IQA框架。该框架通过构建记忆库检索相似样本，将VLM转化为概率比较器进行重排序，并结合瑟斯顿模型融合序数证据与初始得分。实验证明，该方法能显著提升预测的灵敏度与准确性，在多个基准测试中优于现有基线。
selection_source: fresh_fetch
motivation: 推理型VLM在IQA任务中生成的标量分数缺乏灵敏度，且容易出现分数集中在少数数值的“离散坍缩”现象。
method: 提出一种即插即用的测试时增强框架，利用推理摘要检索记忆库邻居，通过概率对等比较和瑟斯顿模型进行重排序与分数融合。
result: 在多个IQA基准测试中，该方法显著缓解了离散坍缩问题，并取得了优于强推理基线和非推理方法的性能。
conclusion: ME-IQA通过引入记忆增强和重排序机制，有效提升了VLM在图像质量评估任务中的细粒度区分能力和预测稳定性。
---

## 摘要
推理性视觉语言模型（VLMs）通过文本推理推动了图像质量评估（IQA）的发展，然而其标量评分往往缺乏灵敏度，并会坍缩为少数几个数值，即所谓的“离散坍缩”（discrete collapse）。我们提出了 ME-IQA，一种即插即用的测试时增强记忆重排序框架。该框架：(i) 构建记忆库并利用推理摘要检索语义和感知对齐的邻居；(ii) 将 VLM 重新定义为概率比较器以获取成对偏好概率，并在瑟斯顿第五类模型（Thurstone's Case V model）下将此序数证据与初始评分融合；(iii) 执行门控反思并巩固记忆以改进未来的决策。这产生了更密集、对失真敏感的预测，并缓解了离散坍缩。在多个 IQA 基准测试上的实验表明，该方法相比强推理性 VLM 基线、现有的非推理性 IQA 方法以及测试时缩放替代方案，均取得了持续的性能提升。
