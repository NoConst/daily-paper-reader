...
tags: ["query:cir-ml"]
score: 8.0
evidence: 视觉引导的视觉语言模型提示学习
tldr: 针对视觉语言模型在标签噪声下提示学习鲁棒性差的问题，本文提出 VisPrompt 框架。该方法利用视觉信息比文本标签更稳定的特性，通过跨模态注意力机制将视觉语义注入提示表示，并引入条件调制机制动态平衡文本先验与视觉证据。VisPrompt 在保持预训练模型冻结的同时，有效抑制了噪声干扰，在多个基准数据集上显著提升了模型的鲁棒性和性能。
source: arxiv
...
motivation: 提示学习在标签噪声下极易受干扰，而视觉内容相比文本标签具有更稳定和丰富的语义信息。
method: 采用跨模态注意力机制将视觉特征反向注入提示，并结合轻量级条件调制机制自适应调节视觉信息的注入强度。
result: 在 7 个合成及真实噪声数据集上的实验证明，VisPrompt 在极低参数增量下显著优于现有基准方法。
conclusion: 视觉引导的跨模态提示学习能有效缓解模型对噪声标签的记忆，是提升视觉语言模型鲁棒性的有效途径。
---

## 摘要
提示学习（Prompt learning）是视觉语言模型中一种参数高效的方法，但其在标签噪声下的鲁棒性研究较少。视觉内容包含更丰富且更可靠的语义信息，在标签噪声下表现出更强的鲁棒性。然而，提示本身极易受到标签噪声的影响。基于这一直觉，我们提出了 VisPrompt，这是一个针对噪声标签场景的轻量级且鲁棒的视觉引导提示学习框架。具体而言，我们利用跨模态注意力机制将视觉语义反向注入到提示表示中。这使得提示标记（prompt tokens）能够选择性地聚合与当前样本相关的视觉信息，从而通过将提示学习锚定在稳定的实例级视觉证据上并减少噪声监督的影响，来提高鲁棒性。为了解决由于对所有样本采用相同的视觉信息注入方式（尽管其视觉线索质量存在差异）而导致的不稳定性，我们进一步引入了一种轻量级的条件调制机制，以自适应地控制视觉信息注入的强度，从而在文本侧语义先验与图像侧实例证据之间取得更鲁棒的平衡。所提出的框架有效地抑制了噪声引起的干扰，减少了提示更新中的不稳定性，并缓解了对错误标注样本的记忆。VisPrompt 在保持预训练视觉语言模型（VLM）骨干网络冻结且仅引入少量额外可训练参数的情况下，显著提高了鲁棒性。在合成和真实世界标签噪声下的广泛实验表明，VisPrompt 在七个基准数据集上普遍优于现有的基准方法，并表现出更强的鲁棒性。我们的代码已在 https://github.com/gezbww/Vis_Prompt 公开。
