...
tldr: 针对文本-视频检索中现有方法忽略视频帧间上下文导致文本与视频语义不匹配的问题，本文提出EagleNet。该模型通过细粒度关系学习（FRL）构建文本-帧图，捕捉帧间交互并生成富含上下文的文本嵌入。同时引入能量感知匹配（EAM）优化交互建模，并采用Sigmoid损失提升对齐稳定性。实验证明EagleNet在多个主流数据集上达到领先水平。
selection_source: carryover_cache
motivation: 现有文本增强方法忽略了视频内部帧之间的交互，导致生成的扩展文本无法捕捉帧的上下文信息，造成文本与视频间的语义差异。
method: 提出FRL机制构建文本-帧图以聚合上下文信息，并设计EAM模块建模交互能量，同时使用Sigmoid损失替代传统对比损失。
result: 在MSRVTT、DiDeMo、MSVD和VATEX等多个权威数据集上的实验结果均证明了EagleNet的优越性。
conclusion: EagleNet通过细粒度的关系学习和能量感知匹配，有效缩小了文本与视频间的语义鸿沟，提升了检索性能。
---
