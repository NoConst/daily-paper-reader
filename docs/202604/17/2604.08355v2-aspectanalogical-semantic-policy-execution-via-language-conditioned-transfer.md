...
tags: ["query:cir-ml"]
score: 7.0
evidence: 通过语言调节适应新型或组合任务变体
tldr: 针对强化学习智能体在结构相似任务中泛化能力差的问题，本文提出ASPECT框架。该方法摒弃了预定义的离散类别系统，采用文本条件的变分自编码器（VAE）实现知识迁移。其核心创新在于利用大语言模型（LLM）作为动态语义算子，在测试时将新任务描述重映射为源任务描述，从而生成与原始训练兼容的想象状态，实现零样本策略复用，显著提升了智能体在复杂类比任务中的适应性。
selection_source: fresh_fetch
motivation: 现有的零样本迁移方法受限于预定义的离散类别系统，难以应对新颖或复合的任务变体。
method: 利用LLM作为语义算子将新任务描述重映射至源任务，并结合文本条件VAE生成兼容的想象状态以复用原始策略。
result: 该方法在多种复杂且真实的类比任务中实现了高效的零样本迁移，超越了固定类别映射的局限性。
conclusion: 通过结合LLM的灵活推理与文本条件生成模型，ASPECT为强化学习提供了一种更具通用性的类比语义迁移方案。
---
