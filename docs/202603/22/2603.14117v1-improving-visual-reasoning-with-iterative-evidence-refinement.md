...
tldr: "视觉语言模型在复杂推理中常需重新审视图像细节，但现有方法依赖外部缩放或裁剪工具，效率较低。本文提出SIEVE框架，通过内部表征实现端到端的视觉证据重访。该方法利用强化学习训练模型在推理过程中自动识别并注入关键区域的嵌入向量，无需重新编码图像。实验表明，SIEVE在多个基准测试中平均提升了8%的性能，有效增强了模型的感知与推理能力并减少了幻觉。"
selection_source: fresh_fetch
motivation: 针对现有视觉语言模型在推理过程中重新获取细粒度视觉细节时依赖外部工具、导致推理中断且效率低下的问题。
method: 提出SIEVE框架，通过强化学习训练模型自主决定何时重访视觉证据，并提取关键区域的内部嵌入直接注入推理链。
result: "在多个视觉推理和幻觉评估基准上，SIEVE相比基线模型实现了平均8%的性能增长。"
conclusion: 证明了利用模型内部信号进行迭代视觉证据细化是提升视觉语言模型推理鲁棒性的有效途径。
---
