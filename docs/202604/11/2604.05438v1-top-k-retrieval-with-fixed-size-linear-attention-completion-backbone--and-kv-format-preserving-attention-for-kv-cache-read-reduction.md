...
tldr: 针对长文本生成中KV缓存读取带来的IO瓶颈，本文提出一种保持模型权重和KV格式不变的检索-补全注意力机制。该方法在Top-K检索的基础上，利用预填充阶段生成的固定大小特征图摘要来估算未检索部分的注意力贡献，从而在不增加额外KV读取负担的情况下修正Softmax归一化偏置，显著提升了长文本处理的准确性。
selection_source: fresh_fetch
motivation: 传统的Top-K检索在减少KV缓存读取时会因忽略未检索Token而导致Softmax归一化偏置，影响模型在长上下文下的生成质量。
method: 结合精确计算（针对锚点及Top-K Token）与线性注意力估算（利用预填充时的固定大小特征图总结中间区域），在非归一化域合并两部分贡献。
result: 在相同的Token读取预算下，该方法在长文本基准测试中显著优于纯Top-K检索，尤其在信息熵较高的注意力头上提升巨大。
conclusion: 该方法在不改变模型架构和KV缓存格式的前提下，有效缓解了长文本生成的IO瓶颈并提升了检索精度。
---

## 摘要
长上下文生成日益受到解码阶段键值（KV）缓存传输流量的限制，尤其是当 KV 缓存被卸载到 GPU 显存之外时。查询感知检索（如 Top-K 选择）通过仅加载 KV 对的子集来减少此类流量，但当注意力权重分散在未检索到的标记（tokens）上时，对该子集进行 Softmax 重新归一化会引入偏差。我们提出了一种检索-补全注意力模块，该模块保持骨干网络权重和 KV 缓存格式不变。对于每个查询，我们计算针对 sink/tail 锚点以及查询相关的 Top-K 检索标记的精确注意力，并利用预填充阶段计算的固定大小特征图摘要来估计剩余中间区域的分子和分母。我们在未归一化域中将精确贡献与估计贡献相加，并进行单次归一化，从而在不增加额外注意力端 KV 读取的情况下，恢复缺失的 Softmax 权重。在长上下文基准测试中，该方法在相同的等效标记读取预算下优于仅进行选择的 Top-K 方法，且在具有高熵注意力头的场景中提升最为显著。
