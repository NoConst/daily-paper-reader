...
tags: ["query:cir-ml"]
score: 8.0
evidence: 模态平衡的多模态主动学习
tldr: 针对多模态学习中大规模标注数据成本高昂的问题，本文提出了一种基于强化学习的主动学习框架RL-MBA。该框架通过动态调整模态权重（AMCB）和基于证据融合的样本难度评估（EFDA），解决了传统方法在训练过程中无法适应模态贡献变化和样本难度波动的问题。实验证明，RL-MBA在多个数据集上显著提升了分类准确率和模态公平性。
selection_source: fresh_fetch
motivation: 现有的多模态主动学习方法通常假设模态重要性是固定的，无法适应训练过程中模态贡献和样本难度的动态变化。
method: 提出RL-MBA框架，利用强化学习动态平衡模态贡献，并结合证据融合技术来识别和优先标注高难度、高信息量的样本。
result: 在Food101、KineticsSound和VGGSound等数据集上的实验结果显示，该方法在有限标注预算下一致优于现有基准模型。
conclusion: RL-MBA通过自适应的模态平衡和难度感知策略，有效提升了多模态主动学习的效率和模型性能。
---
