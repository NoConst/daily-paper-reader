...
title_zh: 先净化后对齐：通过噪声多模态教师知识蒸馏实现模态缺失下的鲁棒人体感知
...
## 摘要
鲁棒的多模态人体感知必须克服模态缺失这一关键挑战。两个主要障碍是异构数据之间的表示鸿沟（Representation Gap）以及低质量模态带来的污染效应（Contamination Effect）。这些障碍具有因果联系，因为污染引入的损坏从根本上阻碍了表示差异的缩小。在本文中，我们提出了 PTA，一种新颖的“先净化后对齐”框架，通过元学习与知识扩散的协同整合来解决这种因果依赖。为了净化知识源，PTA 首先采用了一种元学习驱动的加权机制，动态学习降低噪声及低贡献模态的影响。随后，为了对齐不同模态，PTA 引入了一种基于扩散的知识蒸馏范式，其中由净化后的共识形成的信息丰富的纯净教师模型，负责细化每个学生模态的特征。这种“先净化后对齐”策略的最终成果是创建了注入跨模态知识的、极其强大的单模态编码器。在存在显著表示鸿沟和污染效应的大规模 MM-Fi 和 XRF55 数据集上进行的全面实验表明，PTA 达到了最先进的性能，并显著提高了单模态模型在各种模态缺失场景下的鲁棒性。
