...
tags: ["query:cir-ml"]
score: 8.0
evidence: 基于知识的视觉问答中的搜索智能体问题
tldr: 针对知识库视觉问答（KB-VQA）中固定检索流程难以适应多样化问题且检索与推理脱节的问题，本文提出一种基于决策的搜索智能体框架。该方法将求解过程建模为多步决策，智能体可根据当前状态自主选择回答、图像检索、文本检索或生成描述等操作。通过自动收集推理轨迹并进行微调，该方法在InfoSeek和E-VQA数据集上达到了SOTA性能，显著提升了模型处理长尾知识的能力。
selection_source: fresh_fetch
motivation: 传统的检索增强生成方法采用固定的顺序流程，导致检索与推理分离，难以灵活应对复杂的知识性问题。
method: 将KB-VQA重新定义为搜索智能体问题，通过多步决策机制自主选择检索或回答动作，并利用自动生成的推理轨迹进行监督微调。
result: 在InfoSeek和E-VQA基准测试中，该方法一致优于现有基线模型，取得了当前最先进的性能表现。
conclusion: 将KB-VQA建模为主动决策过程能有效对齐检索证据与问题需求，是解决复杂视觉知识推理问题的有效途径。
---
