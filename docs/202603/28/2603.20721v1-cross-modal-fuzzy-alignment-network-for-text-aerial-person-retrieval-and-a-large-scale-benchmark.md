...
title_zh: 用于文本-航拍行人检索的跨模态模糊对齐网络及大规模基准测试
...
tldr: 本研究针对无人机视角下文本-行人检索中视觉信息退化和语义对齐困难的问题，提出了一种跨模态模糊对齐网络（CFAN）。该网络利用模糊逻辑量化特征可靠性以实现细粒度对齐，并引入地面视角图像作为桥梁缓解模态间隙。此外，构建了大规模基准数据集AERI-PEDES，通过思维链提升文本质量。实验证明该方法在多个数据集上表现优异。
selection_source: fresh_fetch
motivation: 无人机拍摄的行人图像受视角和高度变化影响导致视觉信息退化，使得航拍图像与文本描述之间的语义对齐极具挑战性。
method: 提出模糊令牌对齐模块以增强细粒度对齐的鲁棒性，并设计上下文感知动态对齐模块，利用地面视角图像作为中介来缩小模态差距。
result: 在自建的大规模数据集AERI-PEDES和TBAPR数据集上的实验结果均证明了该方法在文本-航拍行人检索任务中的领先性能。
conclusion: 通过模糊逻辑处理不确定性并引入多视角辅助对齐，能有效解决航拍场景下的跨模态检索难题。
---

## 摘要
文本-航拍行人检索旨在根据目击者描述在无人机捕获的图像中识别目标，支持智能交通和公共安全应用。与地面视角的文本-图像行人检索相比，由于视角和飞行高度的剧烈变化，无人机捕获的图像往往面临视觉信息退化的问题，使得与文本描述的语义对齐变得极具挑战性。为了解决这一问题，我们提出了一种新型的跨模态模糊对齐网络，该网络通过模糊逻辑量化词元级（token-level）可靠性以实现准确的细粒度对齐，并引入地面视角图像作为桥接代理，以进一步缩小航拍图像与文本描述之间的差距。具体而言，我们设计了模糊词元对齐模块，利用模糊隶属度函数动态建模词元级关联强度，并抑制不可观测或噪声词元的影响。该模块可以缓解由视觉线索缺失引起的语义不一致，并显著增强词元级语义对齐的鲁棒性。此外，为了进一步缩小航拍图像与文本描述之间的差距，我们设计了一个上下文感知动态对齐模块，将地面视角代理作为文本-航拍对齐中的桥梁，并自适应地结合直接对齐和代理辅助对齐以提高鲁棒性。此外，我们通过使用思维链（chain-of-thought）将文本生成分解为属性解析、初始描述和精炼，构建了一个名为 AERI-PEDES 的大规模基准数据集，从而提升了文本的准确性和语义一致性。在 AERI-PEDES 和 TBAPR 上的实验证明了我们方法的优越性。
