...
tldr: 视觉语言模型（如CLIP）在处理否定词（如“没有标志”）时表现不佳。本文指出传统检索评估指标的局限性，提出利用多模态大模型作为评判者的新框架。研究发现CLIP嵌入空间中存在特定的“否定方向”，通过表征工程技术在无需微调的情况下即可干预模型行为，显著提升其对否定语义的理解能力，并验证了该方法在分布偏移下的泛化性。
selection_source: fresh_fetch
motivation: 现有的CLIP模型难以理解文本查询中的否定语义，且传统的检索评估指标无法准确衡量模型对否定的真实理解程度。
method: 提出基于多模态大模型的评估框架，并利用表征工程在CLIP嵌入空间中识别并操纵与否定相关的几何方向。
result: 发现无需微调即可通过测试时干预引导CLIP理解否定，且该方法在非通用图像文本样本上表现出良好的泛化能力。
conclusion: 否定理解在视觉语言模型中可以被视为一个几何问题，通过操纵嵌入空间的特定方向能有效提升模型的否定处理能力。
---
