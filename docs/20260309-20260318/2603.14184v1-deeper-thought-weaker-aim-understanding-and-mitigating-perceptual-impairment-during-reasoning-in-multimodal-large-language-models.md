...
title_zh: 思虑越深，目标越弱：理解并缓解多模态大语言模型推理过程中的感知受损
...
## 摘要
多模态大语言模型（MLLMs）在长链推理模式下经常遭受感知受损，特别是在视觉问答（VQA）任务中。我们发现注意力分散是其根本原因：在多步推理过程中，模型的视觉注意力变得分散并偏离了与问题相关的区域，实际上对视觉输入“失去了焦点”。为了更好地理解这一现象，我们分析了 MLLMs 的注意力图，并观察到推理提示显著降低了对回答问题至关重要的区域的注意力。我们进一步发现，模型对图像标记（tokens）的整体注意力与其在图像内部注意力的空间分散性之间存在强相关性。利用这一发现，我们提出了一种无需训练的视觉区域引导注意力（VRGA）框架，该框架基于熵聚焦准则选择视觉头并重新加权其注意力，从而在推理过程中有效地引导模型关注与问题相关的区域。在视觉语言基准测试上的广泛实验表明，我们的方法有效地缓解了感知退化，提高了视觉定位和推理的准确性，同时为 MLLMs 如何处理视觉信息提供了可解释的见解。
