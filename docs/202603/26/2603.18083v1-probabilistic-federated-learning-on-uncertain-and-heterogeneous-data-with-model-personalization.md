...
tags: ["query:cir-ml"]
score: 7.0
evidence: 使用元学习和贝叶斯神经网络的个性化联邦学习
tldr: 针对联邦学习中数据不确定性和异构性导致的训练退化问题，本文提出 Meta-BayFL 框架。该方法创新性地结合了贝叶斯神经网络（BNN）与元学习，通过在隐藏层引入不确定性建模并利用自适应学习率进行个性化更新，有效提升了在非独立同分布及噪声数据下的模型性能。实验证明其在多个数据集上显著优于现有主流个性化联邦学习方法，且兼顾了计算与通信效率。
selection_source: fresh_fetch
motivation: 传统联邦学习难以应对客户端数据的不确定性和异构性，且现有概率方法在联邦场景下的额外开销问题亟待解决。
method: 提出 Meta-BayFL 框架，融合 BNN 建模不确定性与元学习实现个性化自适应更新，并优化了全局模型的聚合机制。
result: "在 CIFAR-10/100 和 Tiny-ImageNet 数据集上，Meta-BayFL 的测试准确率比 pFedMe、Ditto 等先进方法高出多达 7.42%。"
conclusion: Meta-BayFL 通过结合概率建模与个性化元学习，为资源受限环境下的鲁棒联邦学习提供了一种高效且具有理论收敛保障的解决方案。
---
