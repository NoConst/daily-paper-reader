...
tldr: 本研究针对经验测度（加权点云）提出了一种稠密联想记忆模型。通过最小化基于去偏 Sinkhorn 散度的 Hopfield 型能量函数，研究者推导出了球面 Hellinger Kantorovich (SHK) 梯度流作为检索动力学，能够同时更新点云的位置和权重。该方法在理论上保证了几何收敛性和指数级的存储容量，实验证明其在处理点云扰动检索时比传统欧氏距离模型更具鲁棒性。
selection_source: fresh_fetch
motivation: 旨在解决传统联想记忆模型无法有效处理具有加权点云结构的经验测度数据检索问题。
method: 利用去偏 Sinkhorn 散度构建能量函数，并采用球面 Hellinger Kantorovich 梯度流来同步优化支持点位置与权重。
result: 理论证明了该模型具备局部收敛性、盆地不变性及指数级存储容量，并在实验中展现出对点云扰动的高鲁棒性。
conclusion: 该研究为测度空间下的联想记忆检索提供了结合最优传输理论的严谨数学框架与高效算法。
---
