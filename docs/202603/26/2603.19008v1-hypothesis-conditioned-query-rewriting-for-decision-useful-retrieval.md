...
tags: ["query:cir-ml"]
score: 7.0
evidence: 面向证据的检索查询重写
tldr: 针对检索增强生成（RAG）在决策任务中仅关注主题相关性而忽视证据区分性的问题，本文提出HCQR框架。该方法无需训练，通过从问题和选项中提取假设，并将其重写为支持假设、区分选项和验证线索的三类针对性查询，将检索目标从“主题导向”转向“证据导向”。在医学问答数据集上的实验证明，HCQR显著提升了答案选择的准确率，为复杂决策场景提供了更有效的检索策略。
selection_source: fresh_fetch
motivation: 传统的RAG检索往往只关注主题相关性，难以提供足以区分不同候选选项的关键决策证据。
method: 提出HCQR框架，通过生成工作假设并将其重写为支持、区分和验证三类特定查询，实现精准的证据检索。
result: 在MedQA和MMLU-Med数据集上，HCQR相比基础RAG分别提升了5.9和3.6个百分点，表现优于重排序等基准方法。
