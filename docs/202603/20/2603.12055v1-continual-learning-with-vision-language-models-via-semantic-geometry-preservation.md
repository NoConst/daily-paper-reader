...
motivation: 现有方法在适应新任务时未能显式保护预训练的跨模态语义几何，导致新任务监督引起严重的几何畸变和遗忘。
method: 提出SeGP-CL框架，利用对抗锚点探测漂移区域，并通过几何蒸馏、文本正则化及双路径推理机制来保持跨模态结构的稳定性。
result: 在五个持续学习基准测试上的广泛实验表明，SeGP-CL在保持语义几何一致性的同时，一致性地超越了现有SOTA方法。
