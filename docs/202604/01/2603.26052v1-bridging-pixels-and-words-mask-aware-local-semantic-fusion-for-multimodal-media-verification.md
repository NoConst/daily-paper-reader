...
motivation: 传统的全局融合方法存在“特征稀释”问题，导致模型难以捕捉多模态数据中细微的局部语义不一致。
method: 提出MaLSF框架，利用掩码-标签对作为锚点，通过双向跨模态验证模块和层次化语义聚合模块实现主动的局部冲突检测。
result: 在DGM4和多模态虚假新闻检测任务上均取得了当前最先进的性能表现，验证了该方法的有效性。
conclusion: MaLSF通过模拟人类认知的交叉验证机制，有效解决了多模态验证中的局部冲突识别难题，增强了模型的鲁棒性和解释力。
---

## 摘要
随着多模态虚假信息的手段日益复杂，对其进行检测和定位（grounding）变得至关重要。然而，目前依赖被动整体融合的多模态验证方法在应对复杂的虚假信息时显得力不从心。由于“特征稀释”现象，全局对齐往往会抹平细微的局部语义不一致性，从而有效地掩盖了其原本旨在发现的冲突。我们提出了 MaLSF（掩码感知局部语义融合），这是一个将范式转向主动双向验证的新型框架，旨在模拟人类认知的交叉引用过程。MaLSF 利用掩码-标签对（mask-label pairs）作为语义锚点，以连接像素和文字。其核心机制包含两项创新：1) 双向跨模态验证（BCV）模块，该模块充当询问者，利用并行查询流（文本作为查询和图像作为查询）来明确指出冲突；2) 分层语义聚合（HSA）模块，该模块智能地聚合这些多粒度的冲突信号，以进行特定任务的推理。此外，为了提取细粒度的掩码-标签对，我们引入了一组多样化的掩码-标签对提取解析器。MaLSF 在 DGM4 和多模态虚假新闻检测任务上均达到了最先进的性能。广泛的消融实验和可视化结果进一步验证了其有效性和可解释性。
