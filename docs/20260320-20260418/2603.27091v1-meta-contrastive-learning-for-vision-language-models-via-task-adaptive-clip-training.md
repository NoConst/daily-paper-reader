...
tags: ["query:cir-ml"]
score: 8.0
evidence: 通过任务自适应训练进行视觉语言模型的元对比学习
tldr: 本文提出领域条件元对比学习框架，旨在提升视觉语言模型的跨领域泛化能力。该方法将学习过程建模为双层元学习问题，通过领域嵌入调制特征表示，并利用梯度更新实现快速领域适应。结合跨领域对齐正则化，该框架在自然与医学等异构数据集上显著增强了模型的鲁棒性与少样本学习性能，为可扩展多模态学习提供了新方向。
source: arxiv
selection_source: fresh_fetch
motivation: 现有的对比学习模型（如CLIP）依赖全局目标函数，未能显式处理领域偏移，导致跨领域泛化性能受限。
method: 提出一种双层元学习框架，通过领域嵌入调制特征表示，并结合内循环梯度更新与跨领域对齐正则化。
result: 该方法在自然和医学等异构数据集上实现了更强的鲁棒性和更优异的少样本自适应表现。
conclusion: 领域条件元学习是提升大规模多模态模型在复杂分布下泛化能力的有效途径。
---

## 摘要
我们提出了领域条件元对比学习（Domain-Conditioned Meta-Contrastive Learning），这是一个旨在提高视觉-语言模型跨域泛化能力的框架。虽然像 CLIP 这样的对比模型通过大规模训练取得了强大的性能，但它们依赖于全局目标函数，未能显式地考虑领域偏移（domain shift）。为了解决这一局限性，我们将多模态学习建模为基于领域条件任务的双层元学习（bilevel meta-learning）问题。具体而言，我们引入了用于调节图像和文本表示的领域嵌入（domain embeddings），并通过基于梯度的内循环更新（inner-loop updates）优化模型，使其能够快速适应特定领域的分布。此外，我们结合了跨域对齐正则化，以促进领域不变表示（domain-invariant representations）的生成。我们的方法与标准的对比训练流程兼容，并可应用于涵盖自然和医学领域的异构数据集。我们预期该方法能提高在领域偏移下的鲁棒性，并增强少样本自适应性能，为可扩展的多模态学习指明了一个极具前景的方向。
