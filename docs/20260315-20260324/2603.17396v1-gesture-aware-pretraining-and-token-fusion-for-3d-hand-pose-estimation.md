...
title_zh: 面向三维手部姿态估计的手势感知预训练与 Token 融合
...
## 摘要
从单目 RGB 图像中估计三维手部姿态是 AR/VR、人机交互和手语理解等应用的基础。在这项工作中，我们关注存在离散手势标签集的场景，并展示了手势语义可以作为三维姿态估计的强大归纳偏置。我们提出了一个两阶段框架：首先是手势感知预训练，利用 InterHand2.6M 的粗粒度和细粒度手势标签学习信息丰富的嵌入空间；随后是以手势嵌入作为中间表示引导的逐关节 Token Transformer，用于最终回归 MANO 手部参数。训练由针对参数、关节和结构约束的分层目标驱动。在 InterHand2.6M 上的实验表明，手势感知预训练在单手准确率上始终优于最先进的 EANet 基准，且这种收益可以在无需任何修改的情况下跨架构迁移。
