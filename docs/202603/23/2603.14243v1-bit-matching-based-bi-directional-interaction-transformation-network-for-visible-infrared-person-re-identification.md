...
tags: ["query:cir-ml"]
score: 7.0
evidence: 用于检索的跨模态表示学习与交互
tldr: 针对可见光-红外行人重识别（VI-ReID）中模态差异大和分布偏移的问题，本文提出了一种双向交互变换网络（BIT）。BIT 摒弃了传统的刚性特征对齐，首次引入基于匹配的策略，通过编码器-解码器架构实现双向特征整合与查询感知评分，显式建模跨模态关联。实验证明 BIT 在多个基准测试上达到了 SOTA 性能，为解决跨模态检索提供了新思路。
selection_source: fresh_fetch
motivation: 现有方法忽视了模态间复杂的隐式关联，且在红外样本稀缺导致的分布偏移下表现不佳。
method: 提出 BIT 网络，利用编码器提取特征并结合解码器进行双向特征集成与查询感知评分，实现显式的成对匹配。
result: 在多个主流 VI-ReID 基准数据集上的实验结果表明，BIT 显著提升了识别精度并达到了当前领先水平。
conclusion: BIT 是首个在 VI-ReID 中引入成对匹配驱动交互的方法，有效缓解了模态鸿沟并提升了跨模态检索性能。
---
