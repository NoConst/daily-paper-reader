...
tldr: 针对传统XML检索系统忽略用户个性化需求、返回结果单一的问题，本文提出一种本体驱动的个性化信息检索框架。该方法整合领域本体和用户画像，将文档、查询和用户特征表示为加权概念向量，并利用语义相似度度量进行匹配。研究强调了底层特定概念的重要性，通过语义关联提升了检索的精准度。实验证明，该框架在查准率和查全率上均优于传统关键词匹配方法，实现了更具适应性的用户中心化检索。
selection_source: fresh_fetch
motivation: 传统XML检索系统无法根据不同用户的知识背景和偏好提供差异化的搜索结果。
method: 结合领域本体与用户画像，将检索对象转化为加权概念向量，并采用语义相似度算法进行细粒度匹配。
result: 实验结果显示，该方法在查准率和查全率方面均显著优于传统的基于关键词的检索技术。
conclusion: 整合本体与用户画像的语义检索框架能有效提升XML文档检索的相关性和个性化水平。
---

## 摘要
本文探讨了改进半结构化可扩展标记语言（XML）文档信息检索的挑战。传统的信息检索系统（IRS）往往忽略了用户的特定需求，尽管用户的知识、偏好和目标各不相同，但对于相同的查询，系统通常返回相同的结果。我们将外部语义资源（即领域本体和用户画像）整合到检索过程中。文档、查询和用户画像被表示为加权概念向量。该本体采用了一种概念加权机制，强调高度具体的概念，因为层次结构中的低层节点能提供更精确且更有针对性的信息。相关性通过语义相似度度量进行评估，这些度量捕捉了超越关键词匹配的概念关系，从而实现了用户画像、查询和文档之间的个性化及细粒度匹配。实验结果表明，将本体与用户画像相结合提高了检索效果，比基于关键词的方法获得了更高的准确率和召回率。总之，所提出的框架增强了 XML 搜索结果的相关性和适应性，支持了更以用户为中心的检索。
