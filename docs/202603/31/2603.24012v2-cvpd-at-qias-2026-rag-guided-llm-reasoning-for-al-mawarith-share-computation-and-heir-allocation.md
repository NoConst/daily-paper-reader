...
tags: ["query:cir-ml"]
score: 6.0
evidence: 结合密集检索和 BM25 的混合检索用于 RAG
tldr: 本研究针对复杂的伊斯兰继承法（Al-Mawarith）计算任务，提出了一种基于检索增强生成（RAG）的LLM推理框架。该方法结合了符号计算器生成的合成数据、混合检索与重排序技术，以及模式约束的输出验证，有效解决了法律规则复杂和数值计算一致性问题。在QIAS 2026竞赛中，该系统以0.935的MIR-E得分荣获冠军，显著提升了阿拉伯语法律推理任务的可靠性。
selection_source: fresh_fetch
motivation: 伊斯兰继承法涉及多阶段法律推理和复杂的规则变体，传统模型难以在确保法律准确性的同时处理数值分配。
method: 采用RAG流水线，结合符号计算器生成的带推理轨迹的合成数据、混合检索重排序及模式约束验证。
result: 该系统在QIAS 2026盲测中获得0.935的MIR-E评分，位列排行榜第一。
conclusion: 检索增强与模式感知生成的结合，能显著提高高精度阿拉伯语法律推理任务的性能与可靠性。
---
