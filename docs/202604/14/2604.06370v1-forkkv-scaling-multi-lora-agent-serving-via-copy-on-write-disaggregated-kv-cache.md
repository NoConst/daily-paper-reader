...
tags: ["query:cir-ml"]
score: 8.0
evidence: 通过共享上下文和 KV 缓存扩展多智能体工作流
...
## 摘要
大语言模型（LLM）的推理服务范式正迅速转向复杂的多智能体工作流，其中专业化智能体在海量共享上下文上进行协作。虽然低秩自适应（LoRA）能够让这些专业化智能体在单个基座模型上高效共存，但它在推理服务过程中引入了严重的显存占用瓶颈。具体而言，独特的 LoRA 激活导致不同智能体之间的键值（KV）缓存产生差异，使得传统的针对共享上下文的前缀缓存（prefix caching）失效。这迫使系统维护冗余的 KV 缓存，迅速使 GPU 容量饱和并降低吞吐量。为了应对这一挑战，我们推出了 ForkKV，这是一个针对多 LoRA 智能体工作流的推理服务系统，其核心是借鉴了操作系统中一种新颖的内存管理范式：带有写时复制（CoW）的 fork。通过利用 LoRA 的结构特性，ForkKV 将 KV 缓存物理地解耦为一个海量的共享组件（类似于父进程的内存页）和轻量级的智能体特定组件（子进程的内存页）。为了支持这一机制，我们提出了一种 DualRadixTree 架构，允许新 fork 的智能体继承海量共享缓存，并对其轻量级的独特缓存应用 CoW 语义。此外，为了保证高效执行，我们设计了 ResidualAttention，这是一种专门的算子内核，可直接在片上 SRAM 中重构解耦的 KV 缓存。在多种语言模型和不同任务的实际数据集上进行的全面评估表明，ForkKV 的吞吐量达到了现有最先进多 LoRA 推理服务系统的 3.0 倍，且对生成质量的影响微乎其微。
