...
tags: ["query:cir-ml"]
score: 8.0
evidence: 可解释的视觉语言推理与显著性对齐
tldr: 针对视觉语言模型（VLM）过度依赖文本线索及产生幻觉的问题，本文提出 Saliency-R1 框架。该框架引入一种高效的显著性图技术来追踪视觉信息流，并以显著性图与人工标注框的重合度作为奖励函数，通过 GRPO 强化学习算法优化模型。实验证明，该方法显著提升了模型推理的忠实度、可解释性及任务性能。
selection_source: fresh_fetch
motivation: 解决视觉语言模型在推理过程中过度依赖文本而忽视视觉证据，导致输出缺乏依据或产生幻觉的信任危机。
method: 提出一种无需额外计算开销的显著性图技术，并利用其与目标区域的重合度作为奖励，通过 GRPO 算法引导模型关注关键视觉区域。
result: 实验结果显示，Saliency-R1 在提高模型推理忠实度和可解释性的同时，也增强了在各类视觉任务上的整体表现。
conclusion: 通过将视觉显著性对齐引入强化学习，可以有效增强视觉语言模型的推理逻辑与视觉上下文的一致性。
---
