...
tldr: "针对多模态语义分割中传感器特性差异大及开发冗余的问题，本文提出通用RGB-X框架SegRGB-X。该模型通过MA-CLIP提供模态感知引导，利用模态对齐嵌入捕获细粒度特征，并结合DSRM进行动态调整。在事件、热成像、深度等五类模态数据集上均达到SOTA性能，mIoU达65.03%，实现了跨模态的统一高效分割。"
selection_source: fresh_fetch
motivation: 旨在解决不同传感器模态特性差异大导致的开发冗余以及多模态语义分割缺乏通用框架的问题。
method: 提出由MA-CLIP模态感知引导、模态对齐嵌入及领域特定细化模块（DSRM）组成的统一分割框架。
result: "在事件、热成像、深度、偏振和光场五种模态的数据集上均取得SOTA性能，平均mIoU达到65.03%。"
conclusion: SegRGB-X通过统一的架构设计和模态感知机制，成功实现了对任意传感器模态的高效语义分割。
---
