...
## 速览
**TLDR**：针对可见光-红外行人重识别中模态差异大且现有重排序算法难以兼顾模态内变化与模态间差异的问题，本文提出了一种渐进式模态关系重排序方法（HHCR）。该方法通过异构一致性模块探索跨模态关系，通过同构一致性模块研究模态内关系，并构建了CRI推理网络基准。实验证明该方法具有良好的泛化性，并在多个数据集上达到了SOTA性能。 \
**Motivation**：现有重排序算法难以同时处理跨模态行人重识别中的模态内变化和模态间显著差异。 \
**Method**：提出包含异构和同构一致性重排序两个模块的HHCR方法，并构建了CRI推理网络基准。 \
**Result**：实验证明该重排序方法具有通用性，且在多个基准测试中均取得了当前最先进的性能。 \
**Conclusion**：HHCR通过渐进式探索模态间与模态内的内在联系，有效解决了跨模态匹配难题并提升了识别精度。
