...
## 摘要
保护隐私的学习系统通常在层级视觉表示中注入噪声；一个核心挑战是以一种可解释且适用于视觉骨干网络和视觉语言模型（VLM）的方式，对这些扰动如何与声明的隐私预算对齐进行“建模”。我们提出了 Bodhi VLM，一种针对“层级神经表示”的“隐私对齐建模”框架：它（1）通过基于 NCP 和 MDAV 的聚类将敏感概念与逐层分组联系起来；（2）在多尺度表示（如特征金字塔或视觉编码器层）上使用自底向上（BUA）和自顶向下（TDA）策略定位敏感特征区域；（3）使用期望最大化隐私评估（EMPA）模块，通过将拟合的敏感特征分布与评估者指定的参考分布（如尺度为 c/ε 的拉普拉斯或高斯分布）进行比较，生成可解释的“预算对齐信号”。输出是相对于参考值的，并非正式的差分隐私估计器。我们在层级特征结构上形式化了 BUA/TDA，并在目标检测器（YOLO、PPDPTS、DETR）以及 VLM 的视觉编码器（CLIP、LLaVA、BLIP）上验证了该框架。BUA 和 TDA 产生了相当的偏差趋势；EMPA 在报告的设置下提供了稳定的对齐信号。我们与通用的差异基准（卡方、K-L、MMD）以及任务相关的基准（MomentReg、NoiseMLE、Wass-1）进行了比较。结果以多个种子的均值±标准差形式报告，置信区间见补充材料。这项工作为隐私对齐的层级表示提供了一个可学习、可解释的建模视角，而不仅仅是事后审计。源代码：https://github.com/mabo1215/bodhi-vlm.git
