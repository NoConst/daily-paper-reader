...
## 摘要
大语言模型在多轮用户交互中难以积累证据，无法以符合贝叶斯推理的方式更新其信念。现有解决方案通常需要对敏感的用户交互数据进行微调，这限制了它们在隐私敏感场景中的适用性。我们提出了 AdaptFuse，这是一个无需训练的框架，它将概率计算完全从大语言模型中外部化：一个符号模块在离散假设集上维持贝叶斯后验，而一个冻结的大语言模型通过多样本狄利克雷聚合（Dirichlet aggregation）提供语义推理。这两种信号通过熵自适应融合进行结合，该方法根据预测置信度自动为每个来源分配权重，随着证据的积累，将依赖从大语言模型逐渐转向符号后验。我们在航班推荐、酒店推荐和网络购物三个领域，以及 Gemma 2 9B、Llama 3 8B 和 Qwen 2.5 7B 模型上进行了评估。实验结果表明，AdaptFuse 在所有任务上均一致优于提示词基线和经过微调的贝叶斯教学（Bayesian Teaching）模型，且准确率随交互轮数单调提升。这些结果证明，有原则的推理时算法可以在个性化推荐中替代微调，且无需存储敏感用户数据或在其上进行训练。所有代码和材料都将开源。
