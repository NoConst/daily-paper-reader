...
## 速览
**TLDR**：LLMind是一个受生物视觉启发的免训练框架，通过模拟人类视觉的中央凹编码和皮层放大效应，实现自适应视觉表示。它引入了莫比乌斯参数化采样策略（BASS）和闭环语义反馈（CSF），在极低像素预算下显著提升了视觉语言模型的效率与性能，且无需修改模型架构，具有极强的通用性。 \
**Motivation**：现有的视觉语言模型对所有视觉区域采用均匀采样，忽略了人类视觉具有选择性和资源高效的自适应特性。 \
**Method**：提出LLMind框架，利用莫比乌斯参数化模块进行非均匀采样，并结合闭环语义反馈使感知显著性与文本信息对齐。 \
**Result**：在多个基准测试中，LLMind在极低像素预算下性能提升显著，仅用5%的像素即可保留97%的全分辨率性能。 \
**Conclusion**：LLMind证明了生物启发式的自适应视觉表示能在不重新训练的情况下，大幅提高视觉语言模型的效率和准确性。
