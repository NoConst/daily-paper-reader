...
tldr: 针对大语言模型智能体在工具调用中因频繁重计算导致的延迟和错误累积问题，本文提出Profile-Then-Reason (PTR) 框架。该框架通过先合成显式工作流再执行、验证及必要时修复的策略，将LLM调用次数限制在2-3次。实验表明，PTR在检索和任务分解类任务上表现优异，在多数配置下优于传统的ReAct模式，显著提升了推理效率和稳定性。
selection_source: fresh_fetch
motivation: 传统的反应式执行模式在每步观察后重复推理，导致高延迟且易受错误传播影响。
method: 提出一种有界执行框架，通过预先规划工作流、确定性执行、结果验证及按需修复的流程来减少模型调用。
result: 在6个基准测试和4个模型上的实验显示，PTR在16/24种配置中优于ReAct，尤其擅长检索和分解密集型任务。
conclusion: PTR通过限制语义复杂度有效提升了工具增强型智能体的性能，但在需要高度在线适应的场景下，反应式执行仍具优势。
---
