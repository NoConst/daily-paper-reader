...
method: "将视觉观测划分为N*N网格并提取语义、颜色和深度信息转化为结构化语言，与指令拼接后输入预训练语言模型进行决策。"
result: 在R2R和RxR基准测试及实地部署中，SOL-Nav在显著减少模型参数和训练数据需求的同时，实现了优异的导航表现。
conclusion: 通过将视觉观测转化为结构化语言，SOL-Nav充分利用了语言模型的推理能力，为实现高效且泛化的具身智能导航提供了新方案。
---
