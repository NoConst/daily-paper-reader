...
tags: ["query:cir-ml"]
score: 8.0
evidence: 使用大语言模型的角色驱动推荐
tldr: 针对会话式推荐系统（SBRS）中匿名会话导致个性化不足的问题，本文提出一种画像驱动的推荐框架。该框架结合大语言模型（LLM）和异构知识图谱（KG），通过两阶段架构提取并利用潜在用户画像。首先利用LLM初始化嵌入并在异构KG上通过无监督学习提取画像，随后将其整合进数据驱动的SBRS中进行候选集生成与重排序。实验证明该方法在处理稀疏和冷启动问题上优于传统序列模型。
source: arxiv
...
motivation: 传统的会话式推荐系统通常假设会话是匿名的，这限制了在数据稀疏或冷启动场景下的个性化推荐能力。
method: 提出一个两阶段框架，先通过异构知识图谱和LLM嵌入学习潜在用户画像，再将画像与LLM特征整合进SBRS进行候选生成与重排序。
result: "在Amazon Books和Amazon Movies & TV数据集上的实验表明，该方法在推荐性能上一致优于仅依赖会话历史的序列模型。"
conclusion: 结合结构化关系信号（KG）与语义信息（LLM）来建模用户画像，能有效提升会话式推荐系统的个性化水平和准确性。
---

## 摘要
会话式推荐系统（SBRS）旨在从交互序列中捕捉用户的短期意图。然而，匿名会话的常见假设限制了个性化水平，特别是在数据稀疏或冷启动的情况下。近期大语言模型（LLM）增强推荐的研究进展表明，LLM 可以生成丰富的物品表示，但由于会话的匿名性，利用 LLM 建模用户画像（Persona）仍然具有挑战性。在这项工作中，我们提出了一个画像驱动的 SBRS 框架，该框架显式建模了从异构知识图谱（KG）中推断出的潜在用户画像，并将其集成到数据驱动的 SBRS 中。我们的框架采用两阶段架构，由个性化信息提取和个性化信息利用组成。在个性化信息提取阶段，我们构建了一个异构知识图谱，整合了与时间无关的用户-物品交互、物品-物品关系、物品-特征关联以及来自 DBpedia 的外部元数据。随后，我们在使用 LLM 衍生物品嵌入初始化的知识图谱上，利用异构深度图互信息（HDGI）目标以无监督方式学习潜在用户画像。在个性化信息利用阶段，学习到的画像表示与 LLM 衍生的物品嵌入被共同纳入改进的数据驱动 SBRS 架构中，以生成相关物品的候选集，随后利用基础序列模型进行重排序，以强调短期会话意图。与以往仅依赖序列建模或基于文本的用户表示的方法不同，我们的方法将用户画像建模建立在源自异构知识图谱的结构化关系信号之上。在 Amazon Books 和 Amazon Movies & TV 数据集上的实验表明，我们的方法始终优于使用会话历史衍生用户嵌入的序列模型。
