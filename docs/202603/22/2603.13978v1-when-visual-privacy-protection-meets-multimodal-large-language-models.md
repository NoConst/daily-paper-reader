...
## 摘要
多模态大语言模型（MLLMs）的兴起以及 GPT-4V 等 MLLM 云服务的广泛使用，引发了人们对视觉数据隐私泄露的极大担忧。由于这些模型通常部署在云服务中，用户需要提交其图像和视频，这带来了严重的隐私风险。然而，如何解决此类隐私问题是一个尚未得到充分探索的问题。因此，在本文中，我们旨在进行一项新的研究，以便在享受 MLLM 服务带来的便利的同时保护视觉隐私。我们针对 MLLM 为“黑盒”的实际情况，即我们只能访问其输入和输出，而不知道其内部模型信息。为了解决这一极具挑战性且紧迫的问题，我们提出了一个新颖的框架。在该框架中，我们精心设计了具有帕累托最优性（Pareto optimality）的学习目标，以寻求视觉隐私与 MLLM 性能之间更好的权衡，并提出了关键历史增强优化（critical-history enhanced optimization），以在黑盒 MLLM 情况下有效地优化该框架。我们的实验表明，该方法在不同的基准测试上都是有效的。
