...
tldr: 针对视觉语言模型在不同视角下对同一物体描述不一致的问题，本文提出了一种统一的记忆增强型视觉语言智能体。该模型在单一自回归框架内集成了数据关联、物体描述和探索策略，利用物体级情节记忆确保跨序列的语义一致性。通过自监督学习和不一致性驱动的策略，该方法在物体描述准确性和自相似性上显著优于现有基准，为具身智能体提供了持久且一致的语义表征能力。
selection_source: carryover_cache
motivation: 现有的视觉语言模型在多视角下对同一物体的描述缺乏一致性，且现有方法多为离线或多阶段流水线，难以实现实时的语义持久性。
method: 提出一种统一的自回归框架，将当前观测、俯视图地图和序列化的物体级情节记忆结合，实现数据关联、描述生成与探索策略的同步处理。
result: "在手动标注的测试集上，该模型在标准描述评分上提升了11.86%，在描述自相似性上提升了7.39%。"
conclusion: 记忆增强的统一框架能有效提升具身智能体在复杂环境中的物体识别持久性和语义一致性，并具备良好的性能扩展性。
---

## 摘要
视觉-语言模型（VLM）在不同视角下对同一物体的描述往往不一致，这阻碍了具身智能体随时间构建一致语义表示的能力。先前的方法通过离线多视图聚合或多阶段流水线来解决不一致性，这些流水线将探索、数据关联和描述学习解耦，对先前观察到的物体的推理能力有限。在本文中，我们引入了一种统一的记忆增强视觉-语言智能体，它在单个自回归框架内同时处理数据关联、物体描述和探索策略。该模型处理当前的 RGB 观测、自顶向下的探索地图以及序列化为物体级标记（tokens）的物体级情景记忆，从而确保在长序列中保持持久的物体身份和语义一致性。为了以自监督方式训练模型，我们利用基于分歧的策略和伪描述模型在逼真的 3D 环境中收集了一个数据集，该模型强制要求多视图描述历史之间的一致性。在手动标注的物体级测试集上的广泛评估表明，与基准模型相比，标准描述分数提高了多达 11.86%，描述自相似性提高了 7.39%，同时通过紧凑的场景表示实现了可扩展的性能。代码、模型权重和数据可在 https://github.com/hsp-iit/epos-vlm 获取。
