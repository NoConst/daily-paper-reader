...
tldr: 针对序列推荐中的长尾项交互稀疏问题，本文提出FAERec框架，旨在通过大语言模型（LLM）增强项目表征。该框架设计了自适应门控机制以动态融合ID与语义嵌入，并引入包含项目级对比和特征级相关性约束的双层对齐策略，以解决不同嵌入空间间的结构不一致。实验表明，FAERec在多个基准模型上均显著提升了长尾项的推荐性能。
selection_source: fresh_fetch
motivation: 旨在解决序列推荐中长尾项交互稀疏导致的表征不准，以及ID与LLM嵌入空间融合不佳和结构冲突的问题。
method: 提出FAERec框架，利用自适应门控融合信息，并通过双层对齐策略及课程学习调度器来增强嵌入的一致性。
result: 在三个公开数据集上的广泛实验证明，该方法在多种主流序列推荐模型上均取得了显著的性能提升。
conclusion: 结合LLM语义知识与双层对齐机制是提升长尾项推荐准确性及解决空间不一致问题的有效途径。
---

## 摘要
序列推荐（SR）通过学习用户的历史交互序列来捕捉用户偏好，并提供个性化建议。在现实场景中，大多数物品的交互非常稀疏，即所谓的长尾物品问题。这一问题限制了模型准确捕捉物品转移模式的能力。为了解决这一问题，大语言模型（LLMs）通过捕捉物品间的语义关系，提供了一种极具前景的解决方案。尽管先前已有研究尝试利用 LLM 生成的嵌入来丰富长尾物品的表示，但仍面临以下局限性：1) 难以有效融合协同信号与语义知识，导致物品嵌入质量欠佳；2) 现有方法忽视了 ID 嵌入空间与 LLM 嵌入空间之间的结构不一致性，导致信号冲突，从而降低了推荐准确性。在本研究中，我们提出了一种面向长尾物品序列推荐的融合与对齐增强框架（FAERec），通过生成连贯融合且结构一致的嵌入来改进物品表示。针对信息融合挑战，我们设计了一种自适应门控机制，用于动态融合 ID 嵌入和 LLM 嵌入。随后，我们提出了一种双层对齐方法来缓解结构不一致性。物品级对齐通过对比学习建立同一物品的 ID 嵌入与 LLM 嵌入之间的对应关系，而特征级对齐则约束了两个嵌入空间中对应维度之间的相关性模式。此外，通过课程学习调度器调整这两个对齐项的权重，以避免对复杂的特征级目标进行过早优化。在三个广泛使用的数据集上，结合多种代表性序列推荐骨干模型进行的广泛实验证明了我们框架的有效性和泛化性。
